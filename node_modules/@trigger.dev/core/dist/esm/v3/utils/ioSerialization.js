...
import { OFFLOAD_IO_PACKET_LENGTH_LIMIT, OTEL_SPAN_ATTRIBUTE_COUNT_LIMIT, imposeAttributeLimits, } from "../limits.js";
...
export async function parsePacketAsJson(value, options) {
    if (!value.data) {
        return undefined;
    }
    switch (value.dataType) {
...
export async function createPacketAttributesAsJson(data, dataType) {
    ...
    switch (dataType) {
        case "application/json": {
            return imposeAttributeLimits(flattenAttributes(data, undefined, OTEL_SPAN_ATTRIBUTE_COUNT_LIMIT));
        }
        case "application/super+json": {
            const { deserialize } = await loadSuperJSON();
            const deserialized = deserialize(data);
            const jsonify = safeJsonParse(JSON.stringify(deserialized, makeSafeReplacer()));
            return imposeAttributeLimits(flattenAttributes(jsonify, undefined, OTEL_SPAN_ATTRIBUTE_COUNT_LIMIT));
        }
        case "application/store": {
            return data;
        }
...
export async function replaceSuperJsonPayload(original, newPayload) {
    ...
    if (meta?.values) {
        const originalUndefinedKeys = Object.entries(meta.values)
            ...
            .map(([key]) => key);
        const overridenUndefinedKeys = originalUndefinedKeys.filter((key) => get(newPayloadObject, key) !== undefined);
